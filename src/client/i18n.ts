...
export const DICT_ZH: Record<string, string> = {
  ...
  'overview.compactReserve': '自动压缩预留：占用达 {pct}% 窗口即会在下一步开始前触发压缩（跟随顶部「自动压缩」条的当前设置）',
  'stats.title': '上下文统计',
  ...
  'tune.title': '自动压缩',
  'tune.hint': '调整内置自动压缩的触发阈值与保留尾部（占上下文窗口百分比）。拖动后点「应用」才写入，下一步开始按新阈值触发；「恢复默认」清除自定义、跟随内置配置',
  'tune.threshold': '触发阈值',
  'tune.thresholdHint': '会话占用（按下一次请求的完整信封估算）达到模型上下文窗口的这个百分比时，在下一步开始前把较早历史自动压缩为摘要（内置默认 80%；设为 100% 基本不再自动触发，上下文溢出时的兜底压缩仍生效）',
  'tune.retain': '保留尾部',
  'tune.retainHint': '每次压缩时原样保留的最近内容，按窗口百分比计（内置默认 16%）。必须低于触发阈值——两者的差距就是一次压缩能释放的空间；差距过小会导致压缩收效甚微、反复触发。仅覆盖默认路由策略：针对特定模型的显式覆盖与绝对 retainTokens 配置不受影响，且调过的尾部会与所有模型的有效阈值交叉校验',
  'tune.default': '默认',
  'tune.defaultHint': '未自定义：跟随内置/预设配置（触发 80%、保留 16%）',
  'tune.off': '已到上限：压力路径基本不再自动触发，上下文溢出时的兜底压缩仍生效',
  'tune.apply': '应用',
  'tune.discard': '放弃',
  'tune.reset': '恢复默认',
  'tune.readOnly': '当前环境的设置为只读，仅展示',
  'tune.unavailable': '设置存储不可用，仅展示',
...
export const DICT_EN: Record<string, string> = {
  ...
  'overview.compactReserve': 'Auto-compact reserve: once usage reaches {pct}% of the window, compaction runs before the next step (follows the Auto-compact strip above)',
  ...
  'tune.title': 'Auto-compact',
  'tune.hint': 'Tune the built-in auto-compact trigger threshold and kept tail (percent of the context window). Dragging only drafts — press Apply to land the values; Reset clears the overrides back to the engine configuration',
  'tune.threshold': 'Trigger at',
  'tune.thresholdHint': 'Once usage (estimated over the whole next-request envelope) reaches this percent of the model\'s context window, older history is compacted before the next step (built-in default 80%; at 100% pressure compaction effectively never fires — context-overflow recovery still applies)',
  'tune.retain': 'Keep tail',
  'tune.retainHint': 'The recent history kept verbatim by each compaction, as percent of the window (built-in default 16%). It must stay below the trigger — the gap between the two is what one compaction frees; a hairline gap frees almost nothing and re-triggers constantly. Tunes the DEFAULT routing policy only: explicit per-model overrides and absolute retainTokens pins still win, and the tuned tail is cross-checked against every model\'s effective trigger',
  'tune.default': 'default',
  'tune.defaultHint': 'Not customized: follows the built-in/preset configuration (trigger 80%, tail 16%)',
  'tune.off': 'At the cap: the pressure path effectively never fires; context-overflow recovery still applies',
  'tune.apply': 'Apply',
  'tune.discard': 'Discard',
  'tune.reset': 'Reset',
  'tune.readOnly': 'Settings are read-only here; display only',
  'tune.unavailable': 'Settings storage unavailable; display only',
