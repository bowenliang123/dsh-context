...
export const DICT_ZH: Record<string, string> = {
  ...
  'stats.balance': '账户余额',
  'stats.balanceTip': 'DeepSeek 账户钱包余额（宿主端实时读取 /user/balance）：与上面的费用估算不同——那是按 models.dev 刊例价估算的花费，这里是账号真实剩余额度。需在 设置 → 模型 里配置 DeepSeek API Key；读不到时不显示该格。',
...
export const DICT_EN: Record<string, string> = {
  ...
  'stats.balanceTip': 'Live DeepSeek account wallet balance, read host-side from /user/balance. Not the cost estimate above — that one prices the family’s usage at models.dev list rates. Needs a DeepSeek API Key under Settings → Models; the cell is hidden when it cannot be read.',
