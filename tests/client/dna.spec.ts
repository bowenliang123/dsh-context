...
// header epoch's bands (system prompt, then tool schemas in producer order)
// lead, the message flow follows in seq order: the order the model reads
// the context, which is also the order items joined it.

import assert from 'node:assert/strict'
import { describe, test } from 'vitest'
import { dnaOf } from '../../src/client/dna'
import type { Assembled } from '../../src/client/assemble'
import type { HeaderRecord, HeaderTool, SurfaceNode } from '../../src/shared/types'

function asm(over: Partial<Assembled>): Assembled {
  return { live: true, header: null, nodes: [], missingLive: 0, approximate: false, ...over }
}
...
describe('dnaOf', () => {
  ...
  test('header epoch bands lead: the system prompt first, then the tool schemas in producer order', () => {
    const items = dnaOf(asm({
      header: header({ systemTokens: 100, tools: [tool('bash', 30), tool('write', 20)] }),
      nodes: [node({ seq: 1 })],
    }))
    ...
    assert.equal(items[0].time, 9000, 'the system band carries the epoch time')
  ...
  test('a mid-session epoch refresh keeps the header bands at the FRONT (the epoch seq is bookkeeping, not prompt position)', () => {
    ...
    const items = dnaOf(asm({
      header: header({ seq: 50, systemTokens: 100, tools: [tool('bash', 30)] }),
